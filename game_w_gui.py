...
def find_best_move(user,comp):
	...
	if len(in_comp_only) != 0:
		...
		for f in freq:
			if f in available:
				return f
score = [0,0,0] # score of the user and the computer
play = 'y'
while play == 'y':
	...
	while len(taken_squares) <= 8 and winner == False:
		...
		if check_winner(user_squares,comp_squares)[1] == True: # Check for winner
			print(check_winner(user_squares,comp_squares)[0])
			if check_winner(user_squares,comp_squares)[0] == "user wins":
				score[0] += 1
				print(f"Human: {score[0]}	Computer {score[1]}")
				play = input("Play agian? y/n")
			elif check_winner(user_squares,comp_squares)[1] == "comp wins":
				score[1] += 1
				print(f"Human: {score[0]}	Computer {score[1]}")
				play = input("Play agian? y/n")
			winner = True
		...
		if len(taken_squares) == 9: # Terminate game if 9 moves have been taken without a winner
			...
			score[2] += 1
			break
			print(f"Human: {score[0]}	Computer {score[1]} 	Draw {score[2]}")
			play = input("Play again? y/n")	